...
// CutSuffix catches adjacent suffix checks and removals without moving effects.
var CutSuffix = prog.FromFile(cutSuffixFile)

var cutSuffixFile = &cop.Func{
	Meta: cop.Meta{
		Name:        "Lint/CutSuffix",
		Description: "use strings.CutSuffix instead of checking and then removing a suffix",
		Severity:    cop.Warning,
	},
	Types: true,
	Scope: outsideFrozenConfig,
	Run: func(p *cop.Pass) {
...
// splitTrimStringsCall matches a two-argument strings.<name> call, such as
// HasSuffix, TrimSuffix, or Join, with no variadic expansion.
