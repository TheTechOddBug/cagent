...
func TestCutSuffixProgram(t *testing.T) {
	t.Parallel()
	offenses := coptest.RunProgram(t, CutSuffix, coptest.ProgramFiles{
...
func runCutSuffix(t *testing.T, src string) []cop.Offense {
	...
	require.NoError(t, err)
	pass := &cop.Pass{Cop: cutSuffixFile, FileSet: fset, File: file, Info: info, Package: pkg}
	cutSuffixFile.Check(pass)
