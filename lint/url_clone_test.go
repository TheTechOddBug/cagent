...
func TestURLCloneScope(t *testing.T) {
	...
	assert.Empty(t, runURLClone(t, "pkg/config/v3/sample.go", urlCloneFixture))
	require.Len(t, runURLClone(t, "pkg/config/latest/sample.go", urlCloneFixture), 1)
}

func TestURLCloneProgram(t *testing.T) {
	t.Parallel()
	offenses := coptest.RunProgram(t, URLClone, coptest.ProgramFiles{"clone.go": urlCloneFixture})
	require.Len(t, offenses, 1)
	assert.Equal(t, "Lint/URLClone", offenses[0].CopName)
...
func runURLClone(t *testing.T, filename, src string) []cop.Offense {
	...
	require.NoError(t, err)
	pass := &cop.Pass{Cop: urlCloneFile, FileSet: fset, File: file, Info: info, Package: pkg}
	urlCloneFile.Check(pass)
