...
func f(xs []int, less func(int, int) bool) { sort.SliceStable(xs, less) }`, false},
	...
	offenses := coptest.RunProgram(t, registeredSharedCop(t, programCops, "Lint/SortStableFunc"), files)
	var got []string
...
`
	...
	assert.Empty(t, coptest.Run(t, SortStableFunc, src), "missing type information must not report")
	registeredSharedCop(t, programCops, "Lint/SortStableFunc")
}
