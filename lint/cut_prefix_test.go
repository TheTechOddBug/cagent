...
func TestCutPrefixProgram(t *testing.T) {
	t.Parallel()
	offenses := coptest.RunProgram(t, CutPrefix, coptest.ProgramFiles{
...
func runCutPrefix(t *testing.T, src string) []cop.Offense {
	...
	require.NoError(t, err)
	pass := &cop.Pass{Cop: cutPrefixFile, FileSet: fset, File: file, Info: info, Package: pkg}
	cutPrefixFile.Check(pass)
