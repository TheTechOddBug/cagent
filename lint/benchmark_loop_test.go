...
func TestBenchmarkLoopScope(t *testing.T) {
	...
	assert.Empty(t, runBenchmarkLoop(t, "pkg/config/v3/sample_test.go", benchmarkLoopFixture))
...
func TestBenchmarkLoopProgram(t *testing.T) {
	t.Parallel()
	offenses := coptest.RunProgram(t, BenchmarkLoop, coptest.ProgramFiles{
...
func runBenchmarkLoop(t *testing.T, filename, src string) []cop.Offense {
	...
	require.NoError(t, err)
	pass := &cop.Pass{Cop: benchmarkLoopFile, FileSet: fset, File: file, Info: info, Package: pkg}
