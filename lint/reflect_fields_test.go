...
func TestReflectFieldsScope(t *testing.T) {
	...
	assert.Empty(t, runReflectFields(t, "pkg/config/v3/sample.go", reflectFieldsFixture))
...
func TestReflectFieldsProgram(t *testing.T) {
	t.Parallel()
	offenses := coptest.RunProgram(t, ReflectFields, coptest.ProgramFiles{"fields.go": reflectFieldsHooksFixture})
	require.Len(t, offenses, 1)
	assert.Equal(t, "Lint/ReflectFields", offenses[0].CopName)
...
func runReflectFields(t *testing.T, filename, src string) []cop.Offense {
	...
	require.NoError(t, err)
	pass := &cop.Pass{Cop: reflectFieldsFile, FileSet: fset, File: file, Info: info, Package: pkg}
	reflectFieldsFile.Check(pass)
