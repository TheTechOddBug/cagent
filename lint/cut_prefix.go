...
// CutPrefix pairs prefix checks with trimming before any intervening mutation.
var CutPrefix = prog.FromFile(cutPrefixFile)

var cutPrefixFile = &cop.Func{
	Meta: cop.Meta{
		Name:        "Lint/CutPrefix",
		Description: "use strings.CutPrefix instead of checking and then removing the same prefix",
...
func cutPrefixRemoval(info *types.Info, expr ast.Expr, check *ast.CallExpr) bool {
	...
	if !ok || slice.Low == nil || slice.High != nil || slice.Slice3 || !cutPrefixSame(info, slice.X, check.Args[0]) {
		return false
	}
