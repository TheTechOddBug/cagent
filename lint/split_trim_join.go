...
// SplitTrimJoin catches suffix-only trimming that needlessly materializes and
...
var SplitTrimJoin = &prog.Func{
	Meta: splitTrimJoinFile.Meta,
	Run: func(p *prog.Pass) {
		// The file runner does not resolve imports; use the shared program load.
		for _, pkg := range p.Program.Packages {
			for _, file := range pkg.Syntax {
				pass := &cop.Pass{Cop: splitTrimJoinFile, FileSet: p.Program.Fset, File: file, Info: pkg.TypesInfo, Package: pkg.Types}
				splitTrimJoinFile.Check(pass)
...
var splitTrimJoinFile = &cop.Func{
	Meta: cop.Meta{
		Name:        "Lint/SplitTrimJoin",
		Description: "use strings.CutLast to trim trailing segments without splitting and rejoining",
