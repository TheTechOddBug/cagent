...
// FieldsSeq catches fields materialized only for one value-only range, allowing
...
var FieldsSeq = &prog.Func{
	Meta: fieldsSeqFile.Meta,
	Run: func(p *prog.Pass) {
		// Resolve imports through the shared program load, not the file runner.
		for _, pkg := range p.Program.Packages {
			for _, file := range pkg.Syntax {
				pass := &cop.Pass{Cop: fieldsSeqFile, FileSet: p.Program.Fset, File: file, Info: pkg.TypesInfo, Package: pkg.Types}
				fieldsSeqFile.Check(pass)
