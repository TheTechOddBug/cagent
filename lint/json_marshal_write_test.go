...
func TestJSONMarshalWriteScope(t *testing.T) {
	...
	assert.Empty(t, runJSONMarshalWrite(t, "pkg/config/v3/sample.go", jsonMarshalWriteFixture))
	require.Len(t, runJSONMarshalWrite(t, "pkg/config/latest/sample.go", jsonMarshalWriteFixture), 1)

	pass := &cop.Pass{Cop: jsonMarshalWriteFile}
	jsonMarshalWriteFile.Check(pass)
...
type Buffer = bytes.Buffer`,
	...
	offenses := coptest.RunProgram(t, JSONMarshalWrite, files)
...
func runJSONMarshalWrite(t *testing.T, filename, src string) []cop.Offense {
	...
	require.NoError(t, err)
	pass := &cop.Pass{Cop: jsonMarshalWriteFile, FileSet: fset, File: file, Info: info, Package: pkg}
	jsonMarshalWriteFile.Check(pass)
