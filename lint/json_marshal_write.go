...
// JSONMarshalWrite advises replacing a single buffered encoding, never a stream.
var JSONMarshalWrite = modernizationCop(jsonMarshalWriteFile)

var jsonMarshalWriteFile = &cop.Func{
	Meta: cop.Meta{
		Name:        "Lint/JSONMarshalWrite",
		Description: "consider jsonv2.MarshalWrite for a single buffered JSON value without a newline",
		Severity:    cop.Warning,
	},
	Types: true,
	Run: func(p *cop.Pass) {
		if p.Info == nil || ast.IsGenerated(p.File) || frozenConfigPath.MatchString(filepath.ToSlash(p.Filename())) {
			return
		}
