...
// BenchmarkLoop loads typed tests because the shared SSA program omits them.
var BenchmarkLoop = &prog.Func{
	Meta: benchmarkLoopFile.Meta,
	Run: func(p *prog.Pass) {
		...
		for _, pkg := range pkgs {
			...
			for _, file := range pkg.Syntax {
				...
				seen[filename] = true
				pass := &cop.Pass{Cop: benchmarkLoopFile, FileSet: p.Program.Fset, File: file, Info: pkg.TypesInfo, Package: pkg.Types}
				benchmarkLoopFile.Check(pass)
...
var benchmarkLoopFile = &cop.Func{
	Meta: cop.Meta{
		Name:        "Lint/BenchmarkLoop",
		Description: "consider testing.B.Loop for simple ResetTimer and b.N benchmarks",
		Severity:    cop.Warning,
	},
	Types: true,
	Run: func(p *cop.Pass) {
		if p.Info == nil || !p.IsTestFile() || ast.IsGenerated(p.File) || frozenConfigPath.MatchString(filepath.ToSlash(p.Filename())) {
			return
		}
