...
Generated images are ordinary files in the owning session's workspace. Absolute,
parent-traversing and home-relative requests redirect to a sanitized workspace
basename with a warning; generated media never writes externally or asks for a
path decision. Relative directories, MIME-derived extensions and collision
suffixes preserve existing files.

Session messages store owner-qualified references and sanitized metadata, not
inline generated bytes. Branching and forking retain the original owner. The
manifest authorizes each reference before resolution; unknown owners, roots or
paths fail closed. The viewing working directory never determines ownership.

After a workspace save and manifest record succeed, the session store keeps a
portable copy. A portability failure warns without removing the workspace file.
Deleting a session removes stored manifest and blob rows, not workspace files.
Session databases migrate in place when opened; compatibility testing belongs
in a disposable store, not an active session database.

Ordinary follow-ups send placeholders, not the stored generated bytes. Explicit
file attachment or tool reads are separate operations.
