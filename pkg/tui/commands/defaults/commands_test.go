...
func newTestParser() *commands.Parser {
	return commands.NewParser(
		commands.Category{Name: "Session", Commands: builtInSessionCommands()},
		commands.Category{Name: "Settings", Commands: builtInSettingsCommands()},
	)
}
...
func TestRemoveByIDsDropsSnapshotCommands(t *testing.T) {
	...
	hasID := func(items []commands.Item, id string) bool {
	...
	parser := commands.NewParser(commands.Category{Name: "Session", Commands: filtered})
...
func TestAttachToolsetRestartCompletion(t *testing.T) {
	...
	var restart, other *commands.Item
	for i := range items {
		switch items[i].ID {
...
func TestAttachEffortCompletion(t *testing.T) {
	...
	var effortItem, other *commands.Item
	for i := range items {
		switch items[i].ID {
		case "session.effort":
			effortItem = &items[i]
