...
func builtInSessionCommands() []commands.Item {
	cmds := []commands.Item{
		{
			ID:           "session.clear",
			Label:        "Clear",
...
func builtInSettingsCommands() []commands.Item {
...
func builtInHelpCommands() []commands.Item {
	return []commands.Item{
		{
...
func builtInFeedbackCommands() []commands.Item {
	return []commands.Item{
		{
...
func visibleOnly(items []commands.Item) []commands.Item {
	visible := make([]commands.Item, 0, len(items))
...
func sortByLabel(items []commands.Item) []commands.Item {
	slices.SortFunc(items, func(a, b commands.Item) int {
...
func removeByIDs(items []commands.Item, ids map[string]bool) []commands.Item {
	out := make([]commands.Item, 0, len(items))
	for _, item := range items {
		if !ids[item.ID] {
			out = append(out, item)
		}
...
// newAgentCommandItem builds the command-palette / slash-command commands.Item for a
...
// AgentCommandMsg.
func newAgentCommandItem(commandName string, cmd types.Command) commands.Item {
	return commands.Item{
...
// newMCPPromptItem builds the command-palette / slash-command commands.Item for a single
...
// argument is missing).
func newMCPPromptItem(promptName string, promptInfo tools.PromptInfo) commands.Item {
...
// toolsetRestartCandidates returns one commands.ArgumentCandidate per toolset of the
...
func toolsetRestartCandidates(source toolsetStatusSource) []commands.ArgumentCandidate {
	...
	candidates := make([]commands.ArgumentCandidate, 0, len(order))
	for _, name := range order {
		s := byName[name]
		candidates = append(candidates, commands.ArgumentCandidate{
			Label:       s.Name,
...
// dependency.
func attachToolsetRestartCompletion(items []commands.Item, source toolsetStatusSource) {
	for i := range items {
		if items[i].ID != "session.toolset.restart" {
			continue
		}
		items[i].CompleteArgument = func() []commands.ArgumentCandidate {
...
// effortCandidates returns one commands.ArgumentCandidate per thinking-effort level
...
func effortCandidates(ctx context.Context, source effortLevelsSource) []commands.ArgumentCandidate {
	levels := source.CurrentAgentThinkingLevels(ctx)
	candidates := make([]commands.ArgumentCandidate, 0, len(levels))
	for _, level := range levels {
		candidates = append(candidates, commands.ArgumentCandidate{Label: string(level)})
	}
	return candidates
}
...
// dropCandidates returns one commands.ArgumentCandidate per file currently attached to
...
func dropCandidates(source attachedFilesSource) []commands.ArgumentCandidate {
	attached := source.AttachedFiles()
	candidates := make([]commands.ArgumentCandidate, 0, len(attached))
	for _, path := range attached {
		candidates = append(candidates, commands.ArgumentCandidate{Label: path})
	}
	return candidates
}
...
func attachDropCompletion(items []commands.Item, source attachedFilesSource) {
	for i := range items {
...
func BuildCommandCategories(ctx context.Context, application *app.App) []commands.Category {
	...
	if len(agentCommands) > 0 {
		items := make([]commands.Item, 0, len(agentCommands))
		for name, cmd := range agentCommands {
			items = append(items, newAgentCommandItem(name, cmd))
		}

		categories = append(categories, commands.Category{
			Name:     "Agent Commands",
			Commands: items,
		})
	}
	...
	if len(mcpPrompts) > 0 {
		mcpCommands := make([]commands.Item, 0, len(mcpPrompts))
	...
	if len(skillsList) > 0 {
		skillCommands := make([]commands.Item, 0, len(skillsList))
		for _, skill := range skillsList {
			skillName := skill.Name
			description := toolcommon.TruncateText(skill.Description, 55)

			skillCommands = append(skillCommands, commands.Item{
				ID:           "skill." + skillName,
				Label:        skillName,
				Description:  description,
				Category:     "Skills",
